...
from .config import EvidenceFilterConfig
...
from .validation import EvidenceFilterValidator


class EvidenceFilterService:
    """Runs the Evidence Filter agent for approved Story Hunter output."""

    def __init__(
        self,
        config: EvidenceFilterConfig,
        llm_client: LLMClient,
        validator: EvidenceFilterValidator | None = None,
        logger: StructuredLogger | None = None,
    ):
        self.config = config
        self.llm_client = llm_client
        self.validator = validator or EvidenceFilterValidator(
    ...
    def run_from_files(self, daily_input_path, match_selection_path, story_hunter_path) -> dict[str, Any]:
        ...
        return self.run(daily_input, match_selection, story_hunter)

    def run(self, daily_input: dict[str, Any], match_selection: dict[str, Any], story_hunter: dict[str, Any]) -> dict[str, Any]:
        production_id = story_hunter.get("production_id", "")
        try:
            self.validator.validate_inputs(daily_input, match_selection, story_hunter)
            base_prompt = self._build_prompt(daily_input, match_selection, story_hunter)
        ...
        for attempt in range(1, attempts_allowed + 1):
            ...
            try:
                ...
                self.validator.validate_output(parsed, story_hunter)
        ...
        return self._structured_error(
            production_id=production_id,
            code="EVIDENCE_FILTER_VALIDATION_FAILED",
            message="Evidence Filter failed after retry",
            issues=last_issues,
            retryable=False,
            attempts=attempts_allowed,
        )

    def _build_prompt(self, daily_input: dict[str, Any], match_selection: dict[str, Any], story_hunter: dict[str, Any]) -> str:
        prompt = self.prompt_loader.load_evidence_filter_prompt()
        ...
        variables = {
            ...
            "surprising_fact": story_hunter.get("surprising_fact", ""),
        ...
        return (
            ...
            + "\n\nReturn only the Evidence Filter JSON object."
        )
    ...
    def _log_attempt(
        ...
        self.logger.log(
            {
                "stage_id": "stage-03-evidence-filter",
                "agent_id": "IF-A03",
                ...
                "next_agent": output.get("next_agent", "IF-A04"),
            }
        )
