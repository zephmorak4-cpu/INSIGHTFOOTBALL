...
class PromptLoader:
    """Loads the Insight Engine prompt section from the official Prompt Library."""

    START = "## 8. Agent 4: Insight Engine"
    END = "## 9. Agent 5: Scriptwriter"

    def __init__(self, prompt_library_path: Path):
        self.prompt_library_path = prompt_library_path

    def load_insight_engine_prompt(self) -> str:
        try:
        ...
        if start == -1 or end == -1 or end <= start:
            raise PromptLoadError("Could not locate Insight Engine prompt section in Prompt Library")
