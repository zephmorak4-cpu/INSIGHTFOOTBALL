...
class PromptLoader:
    """Loads the Match Selector prompt section from the official Prompt Library."""

    START = "## 5. Agent 1: Match Selector"
    END = "## 6. Agent 2: Story Hunter"

    def __init__(self, prompt_library_path: Path):
        self.prompt_library_path = prompt_library_path

    def load_match_selector_prompt(self) -> str:
        try:
        ...
        if start == -1 or end == -1 or end <= start:
            raise PromptLoadError("Could not locate Match Selector prompt section in Prompt Library")
