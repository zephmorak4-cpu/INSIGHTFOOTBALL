"""Command-line entrypoint for Story Hunter sample execution."""
...
from .llm import OpenAICompatibleHTTPClient, RuleBasedStoryHunterClient
from .service import StoryHunterService


def main() -> int:
    parser = argparse.ArgumentParser(description="Run INSIGHT FOOTBALL Story Hunter")
    ...
    parser.add_argument("--config", required=True, help="Path to Story Hunter config JSON")
    ...
    if args.provider == "deterministic" or config.provider == "deterministic":
        client = RuleBasedStoryHunterClient(daily_input, match_selection)
    else:
    ...
    result = StoryHunterService(config, client).run(daily_input, match_selection)
    if args.output:
